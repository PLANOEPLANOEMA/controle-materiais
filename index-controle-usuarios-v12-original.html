...
const predio3D = {
  ...
  computeFloorStatus(items) {
    ...
    // Se houver algum em status de execução/aguardando ou planejado -> Amarelo
    return 'amarelo';
  },
    ...
    if (!selected) {
      ...
      details.innerHTML = selected.items.length ? selected.items.map(item => `
        <div class="predio3d-details-item">
          ...
          <div style="margin-top:.35rem;font-size:.88rem;color:var(--muted);">
            Material: ${item.material || '-'} • Data: ${item.dataServico || item['Início Planejado'] || item.dataNecessidade || '-'}
          </div>
        </div>`).join('') : '<div class="predio3d-empty">Nenhuma RT criada neste pavimento ainda.</div>';
    }
