  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media (max-width: 768px) {
      ...
      .nav-section { flex-direction: row; flex-wrap: wrap; gap: .5rem; align-items: center; }
      .nav-section-title { margin-bottom: 0; }
    }
    ...
    .predio3d-stage { min-height:760px; background:linear-gradient(180deg,#eef4ff 0%,#f9fbff 56%,#eef3f8 100%); border-radius:18px; overflow:hidden; position:relative; box-shadow:var(--shadow); border:1px solid #dbe7f7; }
    ...
    .predio3d-viewport { perspective:1600px; width:100%; min-height:760px; display:flex; align-items:center; justify-content:center; touch-action:none; cursor:grab; user-select:none; }
    .predio3d-viewport.dragging { cursor:grabbing; }
    .predio3d-scene { display:flex; align-items:flex-end; justify-content:center; gap:84px; min-width:100%; min-height:100%; padding:120px 48px 70px; transform-style:preserve-3d; }
    .predio3d-tower-wrap { display:flex; flex-direction:column; align-items:center; gap:.85rem; transform-style:preserve-3d; }
    .predio3d-tower-title { background:rgba(255,255,255,.94); border:1px solid #dbe7f7; border-radius:999px; padding:.45rem .85rem; font-size:.86rem; font-weight:800; color:var(--blue-dark); box-shadow:0 10px 20px rgba(31,56,100,.08); }
    ...
    .predio3d-floor .side { width:56px; height:24px; top:0; opacity:.96; }
    .predio3d-floor .side-left { left:0; transform-origin:left center; transform:rotateY(-90deg); }
    .predio3d-floor .side-right { right:0; transform-origin:right center; transform:rotateY(90deg); }
    .predio3d-floor .roof { width:190px; height:56px; left:0; top:-28px; transform-origin:center bottom; opacity:.96; border-radius:7px 7px 0 0; transform:rotateX(90deg); }
    .predio3d-floor .label-small { opacity:.92; font-size:.72rem; }
    .predio3d-floor.active .front, .predio3d-floor.active .back, .predio3d-floor.active .side, .predio3d-floor.active .roof { outline:2px solid rgba(255,255,255,.96); box-shadow:0 0 0 4px rgba(31,56,100,.16), 0 14px 22px rgba(31,56,100,.22); }
    .predio3d-base { position:absolute; left:50%; bottom:-22px; width:232px; height:40px; background:linear-gradient(180deg,#dbe7f7 0%,#bdd0ea 100%); border-radius:28px; transform:translateX(-50%) translateZ(10px); box-shadow:0 22px 34px rgba(31,56,100,.18); }
    .predio3d-ground { position:absolute; left:50%; bottom:-34px; width:320px; height:56px; background:radial-gradient(circle at center, rgba(140,167,205,.32) 0%, rgba(140,167,205,.16) 55%, rgba(140,167,205,0) 75%); transform:translateX(-50%) rotateX(90deg); }
    .predio3d-verde .front, .predio3d-verde .back, .predio3d-dot.verde { background:#27AE60; }
    .predio3d-verde .roof, .predio3d-verde .side { background:#239a55; }
    .predio3d-amarelo .front, .predio3d-amarelo .back, .predio3d-dot.amarelo { background:#F5A623; color:#1a2340; }
    .predio3d-amarelo .roof, .predio3d-amarelo .side { background:#dc951f; }
    ...
    /* ── APARTAMENTO VIRTUAL 3D DO PAVIMENTO ── */
    .room3d-modal { position:fixed; inset:0; z-index:2200; display:none; align-items:center; justify-content:center; background:rgba(9,18,35,.65); backdrop-filter:blur(8px); padding:18px; }
    .room3d-modal.active { display:flex; }
    .room3d-shell { width:min(1340px,100%); max-height:calc(100vh - 36px); overflow:auto; background:linear-gradient(180deg,#f7fbff 0%,#edf3fb 100%); border-radius:28px; border:1px solid rgba(219,231,247,.96); box-shadow:0 32px 80px rgba(15,35,78,.28); }
    .room3d-topbar { position:sticky; top:0; z-index:5; display:flex; align-items:center; justify-content:space-between; gap:1rem; padding:1rem 1.2rem; background:rgba(247,251,255,.94); border-bottom:1px solid #dbe7f7; backdrop-filter:blur(12px); }
    .room3d-topbar h3 { margin:0; color:var(--blue-dark); font-size:1.24rem; }
    .room3d-topbar p { margin:.2rem 0 0; color:var(--muted); font-size:.92rem; }
    .room3d-content { display:grid; grid-template-columns:minmax(420px,1.2fr) 430px; gap:1.1rem; padding:1.2rem; }
    .room3d-stage { position:relative; min-height:760px; border-radius:28px; overflow:hidden; border:1px solid #dbe7f7; background:radial-gradient(circle at 50% 14%, rgba(255,255,255,.98), rgba(234,241,250,.98) 34%, rgba(215,226,241,.98) 100%); }
    .room3d-stage::before { content:''; position:absolute; inset:0; background:linear-gradient(180deg, rgba(255,255,255,.82), rgba(255,255,255,0) 38%), radial-gradient(circle at 26% 18%, rgba(255,255,255,.8), rgba(255,255,255,0) 34%); pointer-events:none; }
    .room3d-stage::after { content:''; position:absolute; left:50%; bottom:4%; transform:translateX(-50%); width:58%; height:6%; border-radius:50%; background:rgba(83,104,138,.16); filter:blur(18px); pointer-events:none; }
    .room3d-legend-bar { position:absolute; top:16px; left:16px; right:16px; z-index:3; display:flex; flex-wrap:wrap; gap:.55rem; }
    .room3d-legend-pill { display:inline-flex; align-items:center; gap:.38rem; padding:.45rem .78rem; border-radius:999px; background:rgba(255,255,255,.92); border:1px solid #d8e5f7; color:var(--blue-dark); font-size:.78rem; font-weight:700; box-shadow:0 10px 18px rgba(31,56,100,.08); }
    .room3d-scene { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; perspective:1800px; padding:34px; }
    .room3d-apartment { position:relative; width:min(100%,860px); aspect-ratio:1.18 / 1; transform-style:preserve-3d; transform:rotateX(58deg) rotateZ(-41deg) translateY(10px); }
    .room3d-floor-base { position:absolute; inset:20% 10% 10% 10%; border-radius:34px; background:linear-gradient(180deg,#dcc19a 0%,#cda87a 45%,#b78b5e 100%); box-shadow:0 46px 74px rgba(72,91,129,.2), inset 0 14px 20px rgba(255,255,255,.26); }
    .room3d-floor-finish { position:absolute; inset:22% 12% 12% 12%; border-radius:28px; background:linear-gradient(180deg,#d9b88f 0%,#c69c6d 100%); }
    .room3d-floor-lines { position:absolute; inset:22.5% 12.5% 12.5% 12.5%; border-radius:28px; background-image:linear-gradient(rgba(255,255,255,.18) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.18) 1px, transparent 1px); background-size:46px 46px; opacity:.42; }
    .room3d-wall-back, .room3d-wall-left, .room3d-wall-right { position:absolute; border-radius:26px; background:linear-gradient(180deg,#fbfdff 0%,#dbe6f2 100%); box-shadow:inset 0 0 0 1px rgba(156,179,210,.45); }
    .room3d-wall-back { left:16%; right:18%; top:9%; height:24%; }
    .room3d-wall-left { left:10%; top:16%; bottom:14%; width:16%; }
    .room3d-wall-right { right:11%; top:18%; bottom:16%; width:15%; }
    .room3d-ceiling-panel { position:absolute; left:18%; right:20%; top:11%; height:16%; border-radius:22px; background:linear-gradient(180deg,#f6f9fc 0%,#dde8f3 100%); box-shadow:0 18px 28px rgba(95,118,154,.12); opacity:.96; }
    .room3d-ceiling-detail { position:absolute; left:31%; right:33%; top:14.3%; height:7.2%; border-radius:16px; background:linear-gradient(180deg,#ffffff 0%,#e7eef6 100%); box-shadow:inset 0 0 0 1px rgba(190,206,226,.7); }
    .room3d-door-frame { position:absolute; right:18%; bottom:21%; width:13%; height:35%; border-radius:18px 18px 8px 8px; background:linear-gradient(180deg,#d2b38c 0%,#b48757 100%); box-shadow:0 16px 22px rgba(52,43,32,.16); }
    .room3d-door-leaf { position:absolute; inset:4% 8% 3% 8%; border-radius:14px 14px 6px 6px; background:linear-gradient(180deg,#8f623d 0%,#6d472e 100%); box-shadow:inset 0 0 0 1px rgba(46,28,16,.24); }
    .room3d-door-leaf::before { content:''; position:absolute; left:18%; right:18%; top:14%; height:36%; border-radius:12px; border:1px solid rgba(247,231,199,.18); }
    .room3d-door-leaf::after { content:''; position:absolute; right:14%; top:49%; width:10px; height:10px; border-radius:50%; background:#f8deb0; box-shadow:0 0 0 4px rgba(255,255,255,.1); }
    .room3d-window { position:absolute; top:18%; width:18%; height:11%; border-radius:18px; background:linear-gradient(180deg,#e0f2ff 0%,#b8d9f4 100%); border:10px solid rgba(255,255,255,.96); box-shadow:0 18px 24px rgba(82,131,185,.18); }
    .room3d-window.w1 { left:28%; }
    .room3d-window.w2 { left:51%; }
    .room3d-window::before, .room3d-window::after { content:''; position:absolute; background:rgba(255,255,255,.92); }
    .room3d-window::before { width:4px; top:0; bottom:0; left:50%; transform:translateX(-50%); }
    .room3d-window::after { height:4px; left:0; right:0; top:50%; transform:translateY(-50%); }
    .room3d-sofa { position:absolute; left:18%; bottom:23%; width:26%; height:16%; border-radius:26px; background:linear-gradient(180deg,#c5ceda 0%,#97a9be 100%); box-shadow:0 18px 26px rgba(63,80,110,.18); }
    .room3d-sofa::before { content:''; position:absolute; left:6%; right:6%; top:-28%; height:50%; border-radius:18px; background:linear-gradient(180deg,#dce4ee 0%,#b0c1d3 100%); }
    .room3d-rug { position:absolute; left:28%; right:24%; bottom:20%; height:18%; border-radius:50%; background:radial-gradient(circle at center, rgba(232,236,244,.96), rgba(204,214,229,.96)); box-shadow:0 24px 30px rgba(70,94,128,.14); }
    .room3d-table { position:absolute; left:47%; bottom:28%; width:15%; height:10%; border-radius:50%; background:linear-gradient(180deg,#fafbfd 0%,#d8e1ec 100%); box-shadow:0 14px 18px rgba(86,109,145,.14); }
    .room3d-tv { position:absolute; right:24%; top:25%; width:12%; height:8%; border-radius:12px; background:linear-gradient(180deg,#3a465d 0%,#161c28 100%); box-shadow:0 10px 16px rgba(33,43,66,.18); }
    .room3d-plant { position:absolute; left:63%; bottom:22%; width:10%; height:18%; }
    .room3d-plant::before { content:''; position:absolute; left:21%; right:21%; bottom:0; height:34%; border-radius:10px 10px 14px 14px; background:linear-gradient(180deg,#b58e67 0%,#8c6544 100%); }
    .room3d-plant::after { content:''; position:absolute; left:0; right:0; top:0; bottom:18%; border-radius:50% 50% 40% 40%; background:radial-gradient(circle at center, #8bcf8b 0%, #4ea35f 72%); }
    .room3d-hotspots { position:absolute; inset:0; z-index:4; }
    .room3d-surface-btn { position:absolute; display:flex; align-items:center; gap:.45rem; padding:.7rem .9rem; border-radius:18px; border:1px solid #d7e5f7; background:rgba(255,255,255,.94); box-shadow:0 14px 28px rgba(31,56,100,.12); color:var(--blue-dark); font-weight:800; font-size:.84rem; cursor:pointer; transition:transform .18s ease, box-shadow .18s ease, border-color .18s ease; }
    .room3d-surface-btn:hover { transform:translateY(-2px) scale(1.01); box-shadow:0 16px 32px rgba(31,56,100,.16); }
    .room3d-surface-btn:focus-visible { outline:2px solid rgba(0,115,230,.35); outline-offset:2px; }
    .room3d-surface-btn.active { border-color:#0d6efd; box-shadow:0 18px 34px rgba(13,110,253,.18); }
    .room3d-surface-btn .dot { width:10px; height:10px; border-radius:50%; background:#9cb4d3; }
    .room3d-surface-btn.done .dot { background:#27AE60; }
    .room3d-surface-btn.progress .dot { background:#F5A623; }
    .room3d-surface-btn.late .dot { background:#E84545; }
    .room3d-btn-contrapiso { left:13%; bottom:10%; }
    .room3d-btn-gesso { left:8%; top:20%; }
    .room3d-btn-pintura { right:6%; top:30%; }
    .room3d-btn-forro { right:33%; top:7%; }
    .room3d-btn-porta { right:7%; bottom:17%; }
    .room3d-side { display:flex; flex-direction:column; gap:1rem; }
    .room3d-summary { background:#fff; border:1px solid #dbe7f7; border-radius:22px; padding:1rem; box-shadow:0 14px 28px rgba(31,56,100,.08); }
    .room3d-summary-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:.75rem; margin-top:.85rem; }
    .room3d-summary-card { border-radius:16px; background:#f7fbff; border:1px solid #e0ebf8; padding:.85rem; }
    .room3d-summary-card .num { font-size:1.2rem; font-weight:800; color:var(--blue-dark); }
    .room3d-services { display:grid; gap:.85rem; }
    .room3d-service-card { background:#fff; border:1px solid #dbe7f7; border-radius:22px; padding:1rem; box-shadow:0 10px 22px rgba(31,56,100,.07); transition:border-color .18s ease, box-shadow .18s ease, transform .18s ease; }
    .room3d-service-card.active { border-color:#0d6efd; box-shadow:0 18px 34px rgba(13,110,253,.14); transform:translateY(-1px); }
    .room3d-service-head { display:flex; align-items:start; justify-content:space-between; gap:.75rem; margin-bottom:.75rem; }
    .room3d-check { display:flex; align-items:center; gap:.65rem; font-weight:700; color:var(--blue-dark); }
    .room3d-check input { width:18px; height:18px; accent-color:#27AE60; }
    .room3d-status-chip { display:inline-flex; align-items:center; gap:.35rem; padding:.35rem .7rem; border-radius:999px; font-weight:700; font-size:.76rem; background:#eef3fb; color:var(--blue-dark); }
    .room3d-service-fields { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:.75rem; }
    .room3d-service-fields .field-full { grid-column:1 / -1; }
    .room3d-service-fields label { display:block; font-size:.82rem; color:var(--muted); margin-bottom:.32rem; }
    .room3d-service-fields input[type="date"], .room3d-service-fields textarea { width:100%; border:1px solid #d6e4f5; border-radius:14px; padding:.72rem .78rem; font:inherit; background:#fbfdff; }
    .room3d-service-fields textarea { min-height:86px; resize:vertical; }
    .room3d-upload-row { display:flex; flex-wrap:wrap; gap:.55rem; margin-top:.25rem; }
    .room3d-photo-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(104px,1fr)); gap:.65rem; margin-top:.7rem; }
    .room3d-photo-item { position:relative; border-radius:16px; overflow:hidden; min-height:92px; border:1px solid #dbe7f7; background:#eef4fb; }
    .room3d-photo-item img { width:100%; height:100%; object-fit:cover; display:block; }
    .room3d-photo-remove { position:absolute; top:6px; right:6px; border:none; border-radius:999px; width:28px; height:28px; background:rgba(11,20,38,.7); color:#fff; font-weight:800; }
    .room3d-empty-photos { color:var(--muted); font-size:.84rem; padding:.4rem 0 0 0; }
    .room3d-pav-note { margin-top:.6rem; color:var(--muted); font-size:.84rem; }
    @media (max-width:1100px) { .room3d-content { grid-template-columns:1fr; } .room3d-stage { min-height:520px; } }
    @media (max-width:640px) { .room3d-modal { padding:8px; } .room3d-topbar { padding:.9rem; } .room3d-content { padding:.85rem; } .room3d-service-fields, .room3d-summary-grid { grid-template-columns:1fr; } .room3d-surface-btn { font-size:.76rem; padding:.55rem .65rem; } }
    ...
    .mobile-menu {
      ...
      height: 100vh;
      background: var(--card);
      box-shadow: 4px 0 20px rgba(31,56,100,.2);
      z-index: 150;
    ...
    /* ── Responsive ── */
    @media (max-width: 768px) {
      header { flex-direction: column; text-align: center; padding-left: 5rem; }
...
/* ── CONTROLE DE SALDO / NF ── */
.saldo-status{display:inline-flex;align-items:center;gap:.35rem;padding:.35rem .65rem;border-radius:999px;font-weight:800;font-size:.78rem;white-space:nowrap}
.saldo-ok{background:#eaf8f0;color:#1f7a42}.saldo-atencao{background:#fff7df;color:#9a6a00}.saldo-critico{background:#ffe8e8;color:#b42318}.saldo-zerado{background:#f1f5f9;color:#475569}
.saldo-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:1rem;margin:1rem 0}
.saldo-card{background:#fff;border:1px solid #dbe7f5;border-radius:16px;padding:1rem;box-shadow:0 8px 22px rgba(31,56,100,.08)}
.saldo-card .label{color:#6b7a99;font-size:.85rem}.saldo-card .value{font-size:1.55rem;font-weight:900;color:var(--blue-dark);margin-top:.25rem}
.saldo-progress{height:10px;background:#e9f0f8;border-radius:999px;overflow:hidden;margin-top:.6rem}.saldo-progress span{display:block;height:100%;background:linear-gradient(90deg,var(--blue),var(--blue-light));border-radius:999px}
.saldo-alert-box{border-left:5px solid #e84545;background:#fff5f5;padding:.85rem 1rem;border-radius:12px;margin:.6rem 0;color:#7f1d1d}
.saldo-actions{display:flex;gap:.45rem;flex-wrap:wrap}
...
<nav id="desktopNav">
  <!-- Seção RT e Controle de Materiais -->
  <div class="nav-section">
    <div class="nav-section-title">📋 RT & Materiais</div>
    <button class="active" data-section="planejamento-rt" onclick="app.showSection('planejamento-rt', this)">Planejamento RT</button>
    ...
    <button data-section="saldo-nf" onclick="app.showSection('saldo-nf', this)">Saldo por NF</button>
  </div>
  
  <!-- Seção Controle de Romaneios -->
  <div class="nav-section">
    <div class="nav-section-title">📦 Controle de Romaneios</div>
    <button data-section="visao-geral" onclick="app.showSection('visao-geral', this)">Visão Geral</button>
  ...
  <div class="nav-section">
    <div class="nav-section-title">📊 Rastreabilidade & Relatórios</div>
...
<main>
  ...
  <section id="saldo-nf">
    ...
    <div class="card">
      ...
      <div class="filter-bar">
        <input type="text" id="saldoBusca" placeholder="🔍 Buscar material, NF ou fornecedor..." oninput="appSaldoNF.renderizar()" />
        <select id="saldoStatusFiltro" onchange="appSaldoNF.renderizar()">
          <option value="">Todos os status</option>
          <option value="ok">Saldo tranquilo</option>
          <option value="atencao">Atenção</option>
          <option value="critico">Pedir RT/RS</option>
          <option value="zerado">Saldo zerado</option>
        </select>
      </div>
      <div id="saldoResumo" class="saldo-grid"></div>
...
<div id="modalSaldoNF" class="modal">
  <div class="modal-content">
    <div class="modal-topbar"><button type="button" class="btn btn-secondary" onclick="appSaldoNF.fecharModalNF()">← Voltar</button><span style="font-weight:700;color:var(--blue-dark);">Lançamento de NF</span></div>
    <h3 id="saldoNFTitle">Lançar NF</h3>
    <form id="formSaldoNF" onsubmit="appSaldoNF.salvarNF(event)">
...
<div id="room3dModal" class="room3d-modal">
  <div class="room3d-shell">
    <div class="room3d-topbar">
      <div>
        <h3 id="room3dTitle">Apartamento virtual do pavimento</h3>
        <p id="room3dSubtitle">Clique nas superfícies do ambiente para abrir o serviço correto.</p>
      </div>
      <button type="button" class="btn btn-secondary" onclick="floorRooms.close()">✕ Fechar</button>
    </div>
    <div class="room3d-content">
      <div class="room3d-stage">
        <div class="room3d-legend-bar">
          <span class="room3d-legend-pill">🏠 Apartamento visual</span>
          <span class="room3d-legend-pill">🖱️ Parede, teto, chão e porta clicáveis</span>
        </div>
        <div class="room3d-scene">
          <div class="room3d-apartment">
            <div class="room3d-floor-base"></div>
            <div class="room3d-floor-finish"></div>
            <div class="room3d-floor-lines"></div>
            <div class="room3d-wall-back"></div>
            <div class="room3d-wall-left"></div>
            <div class="room3d-wall-right"></div>
            <div class="room3d-ceiling-panel"></div>
            <div class="room3d-ceiling-detail"></div>
            <div class="room3d-window w1"></div>
            <div class="room3d-window w2"></div>
            <div class="room3d-door-frame"><div class="room3d-door-leaf"></div></div>
            <div class="room3d-rug"></div>
            <div class="room3d-sofa"></div>
            <div class="room3d-table"></div>
            <div class="room3d-tv"></div>
            <div class="room3d-plant"></div>
            <div class="room3d-hotspots" id="room3dHotspots"></div>
          </div>
        </div>
      </div>
      <div class="room3d-side">
        <div class="room3d-summary">
          <div style="display:flex;justify-content:space-between;gap:.8rem;align-items:flex-start;">
            <div>
              <strong style="display:block;color:var(--blue-dark);font-size:1rem;">Controle interno do apartamento</strong>
              <div class="room3d-pav-note">Parede esquerda = gesso liso, parede direita = pintura, teto = forro, chão = contra piso e porta = instalação de porta.</div>
            </div>
            <span class="predio3d-badge" id="room3dOverallStatus">Sem dados</span>
          </div>
          <div class="room3d-summary-grid">
            <div class="room3d-summary-card"><div class="num" id="room3dSummaryDone">0</div><div>Concluídos</div></div>
            <div class="room3d-summary-card"><div class="num" id="room3dSummaryProgress">0</div><div>Em andamento</div></div>
            <div class="room3d-summary-card"><div class="num" id="room3dSummaryPhotos">0</div><div>Fotos</div></div>
          </div>
        </div>
        <div id="room3dServices" class="room3d-services"></div>
      </div>
    </div>
  </div>
</div>
    ...
    <form id="formRTDiaDia" onsubmit="appRT.salvarDiaDia(event)">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select id="rtDiaTorre" onchange="appRT.atualizarPavimentosDiaDia()"></select>
        </div>
        <div class="form-group">
    ...
    <form id="formPlanejamentoRT" onsubmit="appRT.salvar(event)">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select id="rtTorre" onchange="appRT.atualizarPavimentosPlanejamento()"></select>
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        <div class="form-group" style="position:relative;">
          <label for="rtCodigoMaterial">Codigo ou Nome do Material *</label>
          <input type="text" id="rtCodigoMaterial" required placeholder="Ex: TQ0-1001 ou Tela Q138" oninput="appRT.buscarMaterialPorCodigoOuNome()" autocomplete="off" />
          <div id="rtMaterialSugestoes" style="position:absolute;top:100%;left:0;right:0;background:#fff;border:1px solid #ddd;border-top:none;max-height:200px;overflow-y:auto;z-index:10;display:none;border-radius:0 0 8px 8px;"></div>
        </div>
        <div class="form-group">
      ...
      <div class="form-row" style="display:none;" id="rtMaterialInfoRow">
        <div class="form-group">
          <label>Material Selecionado</label>
          <div id="rtMaterialInfo" style="padding:0.75rem;background:#f0f8ff;border-radius:8px;border:1px solid #cfe0fb;color:#1f3864;"></div>
        </div>
      </div>
      <input type="hidden" id="rtMaterial" />
      <div class="form-row">
        <div class="form-group">
...
const appSaldoNF = {
  ...
  valorLancado(materialId) { return this.dados.nfs.filter(n => String(n.materialId) === String(materialId)).reduce((s,n)=>s+Number(n.valor||0),0); },
  saldoAtual(mat) { return Number(mat.contrato||0) - this.totalLancado(mat.id); },
  saldoFinanceiro(mat) { return Number(mat.valorContrato||0) - this.valorLancado(mat.id); },
  margemQtd(mat) { return Number(mat.contrato||0) * (Number(mat.margem||0)/100); },
  percentualUsado(mat) { const c=Number(mat.contrato||0); return c>0 ? Math.min(100, (this.totalLancado(mat.id)/c)*100) : 0; },
  status(mat) {
    const saldo = this.saldoAtual(mat), margem = this.margemQtd(mat);
    if (saldo <= 0) return {key:'zerado', label:'Saldo zerado', cls:'saldo-zerado', icon:'⚫'};
    if (saldo <= margem) return {key:'critico', label:'Pedir RT/RS', cls:'saldo-critico', icon:'🔴'};
    if (saldo <= margem*1.5) return {key:'atencao', label:'Atenção', cls:'saldo-atencao', icon:'🟡'};
  ...
  abrirModalMaterial(id=null) {
    ...
    document.getElementById('modalSaldoMaterial').classList.add('active');
  },
  fecharModalMaterial(){ document.getElementById('modalSaldoMaterial').classList.remove('active'); this.editandoMaterialId=null; },
...
const appRT = {
  ...
  carregarMateriais: async function() {
    try {
      const response = await fetch('./Materiais_Codificados.csv');
      const csv = await response.text();
      const linhas = csv.split('\n').slice(1);
      this.materiaisCatalogo = linhas
        .filter(l => l.trim())
        .map(l => {
      ...
      console.warn('Catálogo de materiais não carregado:', e);
    }
  },

  // Buscar material por código ou nome
  buscarMaterialPorCodigoOuNome: function() {
    const input = document.getElementById('rtCodigoMaterial')?.value?.trim() || '';
    const sugestoes = document.getElementById('rtMaterialSugestoes');
    ...
    const resultados = this.materiaisCatalogo.filter(m => 
    ...
    sugestoes.innerHTML = resultados.map(m => `
      <div style="padding:0.75rem;border-bottom:1px solid #eee;cursor:pointer;" 
           onmouseover="this.style.background='#f5f5f5'" 
           onmouseout="this.style.background='transparent'"
           onclick="appRT.selecionarMaterial('${m.codigo}', '${m.nome.replace(/'/g, "\\'")}')">  
  ...
  selecionarMaterial: function(codigo, nome) {
    document.getElementById('rtCodigoMaterial').value = codigo;
    document.getElementById('rtMaterial').value = nome;
    document.getElementById('rtMaterialInfo').innerHTML = `<strong>${codigo}</strong><br>${nome}`;
    document.getElementById('rtMaterialInfoRow').style.display = 'block';
    document.getElementById('rtMaterialSugestoes').style.display = 'none';
  },
  ...
  torresFixas: ['Torre A', 'Torre B'],
  limitesPavimentos: { 'Torre A': 18, 'Torre B': 20 },

  getPavimentosTorre(torre) {
  ...
  normalizarNomePavimento(valor, torre = '') {
  ...
  preencherSelectTorresPavimentos() {
    const preencherTorres = (id) => {
      const el = document.getElementById(id);
      if (!el || el.dataset.ready) return;
      el.innerHTML = '<option value="">Selecione</option>' + this.torresFixas.map(t => `<option value="${t}">${t}</option>`).join('');
      el.dataset.ready = '1';
    };
    preencherTorres('rtTorre');
    preencherTorres('rtDiaTorre');
    this.atualizarPavimentosPlanejamento();
    this.atualizarPavimentosDiaDia();
  },

  atualizarPavimentosPlanejamento() {
    const torre = this.normalizarNomeTorre(document.getElementById('rtTorre')?.value || '');
    const pav = document.getElementById('rtPavimento');
    if (!pav) return;
    const atual = this.normalizarNomePavimento(pav.value, torre);
    const opcoes = torre ? this.getPavimentosTorre(torre) : [];
    pav.innerHTML = '<option value="">Selecione</option>' + opcoes.map(p => `<option value="${p}">${p}</option>`).join('');
    pav.value = opcoes.includes(atual) ? atual : '';
  },

  atualizarPavimentosDiaDia() {
    const torre = this.normalizarNomeTorre(document.getElementById('rtDiaTorre')?.value || '');
    const pav = document.getElementById('rtDiaPavimento');
    if (!pav) return;
    const atual = this.normalizarNomePavimento(pav.value, torre);
    const opcoes = torre ? this.getPavimentosTorre(torre) : [];
    pav.innerHTML = '<option value="">Selecione</option>' + opcoes.map(p => `<option value="${p}">${p}</option>`).join('');
  ...
  preencherSelects() {
    ...
    this.preencherSelectTorresPavimentos();
  ...
  normalizarPlanejamentoImportado(item, idx = 0) {
    ...
    return {
      ...
      torre: this.normalizarNomeTorre(item.Torre || item.torre || ''),
      pavimento: this.normalizarNomePavimento(item.Pavimento || item.pavimento || '', item.Torre || item.torre || ''),
  ...
  normalizarDiaDia(item, idx = 0) {
    ...
    return {
      ...
      torre: this.normalizarNomeTorre(item.torre || ''),
      pavimento: this.normalizarNomePavimento(item.pavimento || '', item.torre || ''),
      quantidade: Number(item.quantidade || 0),
      dataNecessidade,
      leadTime,
  ...
  abrirModalEditar(id) {
    ...
    document.getElementById('rtTorre').value = this.normalizarNomeTorre(item.torre || '');
    this.atualizarPavimentosPlanejamento();
    document.getElementById('rtPavimento').value = this.normalizarNomePavimento(item.pavimento || '', item.torre || '');
  ...
  async atualizarStatus(id, novoStatus) {
    ...
    const anterior = this.itens[idx] || {};
    const item = { ...anterior, status: novoStatus, updatedAt: new Date().toISOString() };
    this.aplicarDatasPorStatus(item, anterior);
    if (novoStatus === 'Entregue' && anterior.status !== 'Entregue' && !anterior.materialEntradaRegistrada) {
      item.materialEntradaRegistrada = true;
      item.dataEntradaEstoque = new Date().toISOString();
      if (window.appMateriais && typeof window.appMateriais.registrarEntradaPorRT === 'function') {
        await window.appMateriais.registrarEntradaPorRT(item, 'planejamento_rt');
      }
    }
    this.itens[idx] = item;
    await this.salvarDados();
  ...
  async atualizarStatusDiaDia(id, novoStatus) {
    ...
    const anterior = this.itensDiaDia[idx] || {};
    const item = { ...anterior, status: novoStatus, updatedAt: new Date().toISOString() };
    this.aplicarDatasPorStatus(item, anterior);
    if (novoStatus === 'Entregue' && anterior.status !== 'Entregue' && !anterior.materialEntradaRegistrada) {
      item.materialEntradaRegistrada = true;
      item.dataEntradaEstoque = new Date().toISOString();
      if (window.appMateriais && typeof window.appMateriais.registrarEntradaPorRT === 'function') {
        await window.appMateriais.registrarEntradaPorRT(item, 'rt_dia_dia');
      }
    }
  ...
  async salvar(event) {
    ...
    const item = {
      ...
      torre: this.normalizarNomeTorre(document.getElementById('rtTorre').value),
      pavimento: this.normalizarNomePavimento(document.getElementById('rtPavimento').value, document.getElementById('rtTorre').value),
    ...
    const anterior = idx >= 0 ? this.itens[idx] : {};
    this.aplicarDatasPorStatus(item, anterior);
    if (item.status === 'Entregue' && anterior.status !== 'Entregue' && !anterior.materialEntradaRegistrada) {
      item.materialEntradaRegistrada = true;
      item.dataEntradaEstoque = new Date().toISOString();
      if (window.appMateriais && typeof window.appMateriais.registrarEntradaPorRT === 'function') {
        await window.appMateriais.registrarEntradaPorRT(item, 'planejamento_rt');
      ...
      item.materialEntradaRegistrada = anterior.materialEntradaRegistrada || false;
      item.dataEntradaEstoque = anterior.dataEntradaEstoque || '';
    }
  ...
  abrirModalNovoDiaDia() {
    ...
    document.getElementById('rtDiaTorre').value = '';
    this.atualizarPavimentosDiaDia();
    document.getElementById('rtDiaPavimento').value = '';
  ...
  abrirModalEditarDiaDia(id) {
    ...
    document.getElementById('rtDiaTorre').value = this.normalizarNomeTorre(item.torre || '');
    this.atualizarPavimentosDiaDia();
    document.getElementById('rtDiaPavimento').value = this.normalizarNomePavimento(item.pavimento || '', item.torre || '');
  ...
  async salvarDiaDia(event) {
    ...
    const item = this.normalizarDiaDia({
      ...
      torre: this.normalizarNomeTorre(document.getElementById('rtDiaTorre').value),
      pavimento: this.normalizarNomePavimento(document.getElementById('rtDiaPavimento').value, document.getElementById('rtDiaTorre').value),
...
<nav class="mobile-menu" id="mobileMenu">
  ...
  <div class="mobile-nav-section">
    <div class="mobile-nav-section-title">📋 RT & Materiais</div>
    <button class="mobile-nav-button active" data-section="planejamento-rt" onclick="app.showSection('planejamento-rt', this); closeMobileMenu()">Planejamento RT</button>
    <button class="mobile-nav-button" data-section="predio-3d" onclick="app.showSection('predio-3d', this); closeMobileMenu()">3D PRÉDIO</button>
    <button class="mobile-nav-button" data-section="rt-dia-dia" onclick="app.showSection('rt-dia-dia', this); closeMobileMenu()">RT Dia a Dia</button>
    <button class="mobile-nav-button" data-section="materiais-obra" onclick="app.showSection('materiais-obra', this); closeMobileMenu()">Controle de Materiais</button>
    <button class="mobile-nav-button" data-section="saldo-nf" onclick="app.showSection('saldo-nf', this); closeMobileMenu()">Saldo por NF</button>
  </div>
  
  <!-- Seção Controle de Romaneios -->
  <div class="mobile-nav-section">
    <div class="mobile-nav-section-title">📦 Controle de Romaneios</div>
    <button class="mobile-nav-button" data-section="visao-geral" onclick="app.showSection('visao-geral', this); closeMobileMenu()">Visão Geral</button>
    <button class="mobile-nav-button" data-section="por-empresa" onclick="app.showSection('por-empresa', this); closeMobileMenu()">Por Empresa</button>
    <button class="mobile-nav-button" data-section="por-material" onclick="app.showSection('por-material', this); closeMobileMenu()">Por Material</button>
    <button class="mobile-nav-button" data-section="registros" onclick="app.showSection('registros', this); closeMobileMenu()">Gerenciar Registros</button>
    <button class="mobile-nav-button" data-section="relatorio" onclick="app.showSection('relatorio', this); closeMobileMenu()">Relatório Visual</button>
  </div>
  
  <!-- Rastreabilidade e Relatórios -->
  <div class="mobile-nav-section">
    <div class="mobile-nav-section-title">📊 Rastreabilidade & Relatórios</div>
    <button class="mobile-nav-button" data-section="rastreabilidade" onclick="app.showSection('rastreabilidade', this); closeMobileMenu()">Rastreabilidade</button>
    <button class="mobile-nav-button" data-section="relatorio-dia" onclick="app.showSection('relatorio-dia', this); closeMobileMenu()">Relatório do Dia</button>
  </div>
</nav>
